...
This application integrates E2B's desktop sandbox with Amazon Nova Pro AI to create an AI agent that can perform tasks
...
export class NovaComputerStreamer
  implements ComputerInteractionStreamerFacade
{
  ...
  constructor(desktop: Sandbox, resolutionScaler: ResolutionScaler) {
    ...
    // Initialize OpenAI client with OpenRouter configuration for Amazon Nova Pro
    this.openai = new OpenAI({
      apiKey: process.env.OPENROUTER_API_KEY || "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",
  ...
  async executeAction(
    ...
    switch (action.action) {
      ...
      case "scroll": {
        ...
        await desktop.scroll(action.direction, action.clicks || 3);
        break;
      }
  ...
  async *stream(
    ...
    try {
      ...
      logError("Error in Nova streaming:", error);
      yield {
        type: SSEEventType.ERROR,
